...
Every artifact is keyed by ``doc_id`` alone (see :mod:`aws_rag.storage`,
:mod:`aws_rag.figures`), so paths and S3 prefixes are derived rather than
looked up. Called from :meth:`aws_rag.backend.local.LocalBackend.delete_doc`
