...
the matching ``aws_rag.store.search`` function, scores the results with
:mod:`aws_rag.eval.metrics`, and returns a per-category + overall report.
...
def run_eval(
    ...
    *,
    ...
    matching (see :mod:`aws_rag.eval.metrics`); the page-based loose number
