...
``datasheet_rag.backend.get_backend``) so the server cannot recurse into a remote
backend even if ``RAG_SERVER_URL`` happens to be set in its environment.

The scope dependencies live here (rather than in :mod:`datasheet_rag.server.auth`)
...
from datasheet_rag.server.auth import (  # noqa: F401  (Scope re-exported)
    KeyContext,
    Scope,
    _bearer,
    auth_enabled,
