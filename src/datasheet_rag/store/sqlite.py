...
:func:`datasheet_rag.store.schema.connect`) and operate in a single transaction
where it makes sense. They mirror the columns declared in
:mod:`datasheet_rag.store.schema`.
...
def delete_doc(conn: sqlite3.Connection, doc_id: str) -> int:
    ...
    :mod:`datasheet_rag.store.schema`) cascades the delete to ``chunk_vecs``
