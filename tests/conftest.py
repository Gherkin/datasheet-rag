...
suite. Pointing ``RAG_HOME`` at a throwaway temp dir *before* ``aws_rag.config``
...
# Must run at import time, before aws_rag.config computes its RAG_HOME default.
_TEST_RAG_HOME = tempfile.mkdtemp(prefix="aws-rag-tests-")
