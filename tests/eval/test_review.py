...
from aws_rag.eval.review import (
    ReviewState,
    apply_update,
    chunks_on_pages,
)
...
def test_apply_update_edits_fields() -> None:
    item = GoldenItem(question="old", category="identifier", doc_id="doc1",
                      gold_pages=[1], source="auto")
    updated = apply_update(item, {
        "question": "new",
        "category": "figure",
        "gold_pages": [14, 15],
...
def test_apply_update_partial_keeps_other_fields() -> None:
    item = GoldenItem(question="q", category="identifier", doc_id="doc1",
                      gold_pages=[3], answer_notes="keep me")
...
def test_review_state_save_roundtrip(conn, tmp_path: Path) -> None:
    ...
    state.eval_set.items[0] = apply_update(state.eval_set.items[0], {
        "gold_pages": [14, 15], "source": "human",
    })
    state.save()
